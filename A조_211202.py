...
class MyApp(QWidget):
    ...
    def Dialog12(self):
        ...
        self.btnSelect2.resize(65, 22)
        self.btnSelect2.clicked.connect(self.new_tree1.select_all)
        self.btnSelect2.clicked.connect(self.new_tree1.get_selected_leaves)
        self.btnSelect2.setStyleSheet('color:white;  background-image : url(./bar.png)')
        font11 = self.btnSelect2.font()
        font11.setBold(True)
        self.btnSelect2.setFont(font11)
        ...
        self.btnUnselect2.resize(65, 22)
        self.btnUnselect2.clicked.connect(self.new_tree1.unselect_all)
        self.btnUnselect2.clicked.connect(self.new_tree1.get_selected_leaves)
        self.btnUnselect2.setStyleSheet('color:white;  background-image : url(./bar.png)')
        font11 = self.btnUnselect2.font()
        font11.setBold(True)
        self.btnUnselect2.setFont(font11)
        ...
        self.btnSelect3.clicked.connect(self.new_tree2.select_all)
        self.btnSelect3.clicked.connect(self.new_tree2.get_selected_leaves)
        ...
        self.btnUnselect3.clicked.connect(self.new_tree2.unselect_all)
        self.btnUnselect3.clicked.connect(self.new_tree2.get_selected_leaves)
            ...
            try:
                ...
                if self.rbtn1.isChecked():
                    ...
                    sql_query = """
                                        ...
                                        ALTER TABLE #tmp add DebitCredit Varchar(25)
                                        UPDATE #tmp set DebitCredit	= CASE When debit = 0 then 'credit' else 'debit' end;
                ...
                if len(self.dataframe) > 1048576:
                    ...
                    if self.rbtn1.isChecked():
                        ...
                        self.scenario_dic[tempSheet + '_Query'] = sql_query + sql_refer
    ...
    def extButtonClicked5_Non_SAP(self):
        ...
        ### 예외처리 4 - 데이터 미추출
            ...
            if self.rbtn1.isChecked():
                ...
                self.scenario_dic[tempSheet_NonSAP + '_Query'] = sql_query
            ...
            if self.rbtn1.isChecked():
                self.scenario_dic[tempSheet_NonSAP + '_Result'] = self.dataframe
                myQuery = self.return_print(sql_query)
                dataframe_1 = {'QUERY': [myQuery]}
                dataframe_1 = pd.DataFrame(dataframe_1)
                self.scenario_dic[tempSheet_NonSAP + '_Query'] = dataframe_1
    ...
    def extButtonClicked11(self):
        ...
        ### 예외처리 2 - 시트명 중복 확인
        elif self.rbtn3.isChecked() and (self.combo_sheet.findText(temp_Sheet + '_Result') != -1 \
                or self.combo_sheet.findText(temp_Sheet + '_Reference') != -1):
            self.alertbox_open5()

        elif self.rbtn4.isChecked() and self.combo_sheet.findText(temp_Sheet + '_Journals') != -1:
            self.alertbox_open5()
        ...
        ### 예외처리 3 - 차대변 선택 오류
            ...
            if self.rbtn3.isChecked():
                ...
                sql_query = '''
                                ...
                                INSERT INTO #filter
                                VALUES
                                ('50004600','Credit','11073000','Debit')

                                --****************CREATE #JEData / Insert ProjectID******************
                                SELECT JENumber,
                                    JELineNumber,
                                    EffectiveDate,
                                    EntryDate,
                                ...
                                --****************CREATE #COAData / Insert ProjectID******************
                                SELECT * INTO #COAData
                ...
                sql_refer = '''
                                ...
                                WHERE JENumber IN (
                                            ...
                                            '''.format(field=self.selected_project_id, CD=temp_State,
                                                       ...
                                                       Account_1=checked_account_11_1, TE=temp_TE,
                                                       YEAR=pname_year)
            ...
            ### 예외처리 4 - 데이터 미추출
                ...
                self.dataframe = pd.DataFrame({'No Data': ['[연도: ' + str(pname_year) + ','
                                                           + '] 라인수 ' + str(len(self.dataframe)) + '개 입니다']})
                ...
                ### JE Line
                if self.rbtn3.isChecked():
                    self.scenario_dic[temp_Sheet + '_Result'] = self.dataframe
                ...
                ### JE
                    ...
                    self.scenario_dic[temp_Sheet + '_Journals'] = self.dataframe
                ...
                if self.rbtn3.isChecked():
                    ...
                    ### 추가 필터링
                    if len(self.dataframe) - 1 <= 500:
                        buttonReply = QMessageBox.information(self, '라인수 추출', str(len(self.dataframe) - 1)
                                                              + '건 추출되었습니다. <br> - TE 금액('
                                                              + str(temp_TE) + ')을 적용하였습니다. <br> [전표라인번호 기준]'
                                                              , QMessageBox.Yes)
                    else:
                        buttonReply = QMessageBox.information(self, '라인수 추출', str(len(self.dataframe) - 1) + '건 추출되었습니다. <br> - TE 금액('
                                                              + temp_TE + ')을 적용하였습니다. 추가 필터링이 필요해보입니다. <br> [전표라인번호 기준]'
                                                              , QMessageBox.Yes)
                ### JE
                elif self.rbtn4.isChecked():
    ...
    def extButtonClicked12(self):
        ...
        if tempSheet == '' or checked_account_12 == 'AND LVL4.GL_Account_Number IN ()':
            self.alertbox_open()
