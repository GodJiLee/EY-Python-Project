...
class MyApp(QWidget):
    ...
    def Dialog4(self):
        ...
        self.dialog4.setGeometry(650, 650, 500, 350)
...
### 배치 - 탭 2 (SAP)============================================================
    ...
    def extButtonClicked4(self):
        ...
        elif self.rbtn1.isChecked() and (self.combo_sheet.findText(tempSheet + '_Result') != -1 or self.combo_sheet.findText(tempSheet + '_Reference') != -1):
            self.alertbox_open5()
                    ...
                    ### JE
                        ...
                        if len(self.dataframe) - 1 <= 500:
                            buttonReply = QMessageBox.information(self, '라인수 추출', '- 계정사용 빈도수 ' + str(temp_N)
                                                                  + '회 이하인 전표가 ' + str(len(self.dataframe) - 1) + '건 추출되었습니다. <br> - TE 금액('
            ...
            try:
                ...
                if self.rbtn1.isChecked():
                    ...
                    sql_query = '''
                            ...
                            '''.format(field=self.selected_project_id, TE=temp_TE_13, CONTI=temp_Continuous, Account=checked_account13)
                ### JE - Journals
                elif self.rbtn2.isChecked():
                ...
                if len(self.dataframe) > 1048576:
                    ...
                    if self.rbtn1.isChecked():
                        ...
                        if len(self.dataframe) - 1 <= 500:
                            buttonReply = QMessageBox.information(self, '라인수 추출',
                                                                  '- 연속된 숫자(' + str(temp_Continuous) + ')로 끝나는 금액을 검토한 결과 '
                                                                  + str(len(self.dataframe) - 1) + '건 추출되었습니다. <br> - TE금액(' + str(temp_TE_13)
                                                                  + ')를 적용하였습니다. 추가 필터링이 필요해보입니다. <br> [전표라인번호 기준]'
                                                                  , QMessageBox.Yes)
